...
const SchedulerPage = () => {
    ...
    useEffect(() => {
        ...
        window.electron.scheduler.onStatus(statusListenerRef.current)

        // 设置日志监听
        logListenerRef.current = (log) => {
            setLogs(prev => [log, ...prev.slice(0, 99)])
        }
        window.electron.scheduler.onLog(logListenerRef.current)

        return () => {
            window.electron.scheduler.removeListener('scheduler:status')
            window.electron.scheduler.removeListener('scheduler:log')
        }
    }, [])

    const loadConfig = async () => {
        try {
            const data = await window.electron.scheduler.getConfig()
            setConfig(data)
    ...
    const loadLogs = async () => {
        try {
            const data = await window.electron.scheduler.getLogs(50)
            setLogs(data)
        } catch (error) {
    ...
    const handleToggleEnabled = async (checked) => {
        ...
        try {
            if (checked) {
                await window.electron.scheduler.enable()
                message.success('定时任务已启用')
            } else {
                await window.electron.scheduler.disable()
    ...
    const handleUpdateConfig = async (key, value) => {
        try {
            await window.electron.scheduler.updateConfig({ [key]: value })
            await loadConfig()
    ...
    const handleExecuteNow = async () => {
        ...
        try {
            const result = await window.electron.scheduler.executeNow()
            if (!result.success) {
    ...
    const handleClearLogs = async () => {
        try {
            await window.electron.scheduler.clearLogs()
            setLogs([])
                        ...
                        <Space direction="vertical" size="large" style={{ width: '100%' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                <div>
                                    ...
                                    <br />
                                    <Text type="secondary">开启后，将在设定时间自动执行解说词获取任务</Text>
                                </div>
                                <Switch
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                ...
                                <TimePicker
                                    ...
                                    format="HH:mm"
                                    onChange={(time) => handleUpdateConfig('executeTime', time?.format('HH:mm') || '08:00')}
                                    style={{ width: 120 }}
                                />
                            </div>
