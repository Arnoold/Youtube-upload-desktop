...
const OwnChannelCommentaryPage = () => {
        ...
        <ConfigProvider locale={zhCN}>
            <div style={{ paddingBottom: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                    <Title level={2} style={{ margin: 0 }}>自有频道解说词任务</Title>
                    <Space>
                        <Popconfirm
                            title="确认关闭"
...
const CreateTaskView = ({ onTaskCreated }) => {
    ...
    const loadVideos = async () => {
        ...
        try {
            ...
            const baseOptions = {
                ...
                sortBy: 'published_at',
                sortOrder: 'desc',
                tableName: 'own_videos' // 指定查询自有频道视频表
            }
        ...
        try {
            ...
            const taskData = {
                ...
                items: videos.map(v => ({
                    video_id: v.video_id,
                    id: v.id,
                    url: v.url || `https://www.youtube.com/watch?v=${v.video_id}`, // 如果没有url字段，则手动构建
                    title: v.title,
                    video_info: v // 保存完整视频信息，方便后续使用
                }))
            }

            // 使用自有频道任务创建接口
            const newTaskId = await window.electron.db.createOwnChannelTask(taskData)
    ...
    const columns = [
        { title: '视频ID', dataIndex: 'id', width: 70 },
        { title: '标题', dataIndex: 'title', ellipsis: true },
        // { title: '频道', dataIndex: 'channel_name', width: 120 }, // 自有频道表无频道名称，暂不显示
    ...
    const defaultFilterValues = {
        ...
        minViews: 0, // 自有频道可能播放量较少，默认0
        generationStatus: ['pending_all'] // 默认只查询待生成的视频
    }
                    ...
                    <Col span={6}>
                        <Form.Item name="generationStatus" label="生成状态">
                            <Select placeholder="全部" allowClear mode="multiple" maxTagCount={2} style={{ width: '100%' }}>
                                ...
                                <Option value="video_deleted">视频已删除</Option>
            ...
            >
                <Form form={createForm} layout="vertical">
                    <Form.Item name="taskName" label="任务名称" rules={[{ required: true, message: '请输入任务名称' }]}>
                        <Input placeholder="例如：2023-10-27 自有频道视频处理" />
                    </Form.Item>
...
const TaskExecutionView = ({ taskId, onBack }) => {
    ...
    const confirmBrowserSelect = () => {
        ...
        // 保存选择到 localStorage
        try {
            localStorage.setItem('own_channel_selected_browsers', JSON.stringify(tempSelectedProfiles))
        } catch (e) {
    ...
    const loadTaskAndItems = async () => {
        ...
        try {
            // 使用自有频道任务接口
            const taskInfo = await window.electron.db.getOwnChannelTaskById(taskId)
            if (!taskInfo) {
                const tasks = await window.electron.db.getOwnChannelTasks()
            ...
            const list = await window.electron.db.getOwnChannelTaskItems(taskId)
            setItems(list)

            // 加载统计信息
            const taskStats = await window.electron.db.getOwnChannelTaskStats(taskId)
            setStats(taskStats)
        } catch (error) {
    ...
    const loadProfiles = async () => {
        try {
            ...
            try {
                const savedSelection = localStorage.getItem('own_channel_selected_browsers')
                if (savedSelection) {
    ...
    const handleStart = async (forceStart = false) => {
        ...
        try {
            // 传递 'own_channel' 作为 taskType
            const result = await window.electron.aiStudio.startTask(taskId, selectedProfiles, 'own_channel')
            if (result.parallel) {
    ...
    const itemColumns = [
        ...
        },
        {
            ...
            render: info => {
                ...
                const videoUrl = info.url || `https://www.youtube.com/watch?v=${info.video_id}`
                return (
                    <a
                        onClick={(e) => {
                            e.preventDefault()
                            e.stopPropagation()
                            window.electron.shell.openExternal(videoUrl)
                        }}
...
const HistoryTab = ({ onLoadTask, isActive }) => {
    ...
    const loadTasks = async () => {
        ...
        try {
            // 使用自有频道任务接口
            const list = await window.electron.db.getOwnChannelTasksWithStats()
            setTasks(list)
        } catch (error) {
    ...
    const handleDelete = async (id) => {
        try {
            // 使用自有频道任务接口
            await window.electron.db.deleteOwnChannelTask(id)
    ...
    const columns = [
        ...
        { title: '任务名称', dataIndex: 'name', ellipsis: true },
        {
            ...
            render: (_, record) => {
                ...
                return (
                    <div>
                        <div style={{ display: 'flex', gap: 8, marginBottom: 4, whiteSpace: 'nowrap' }}>
...
export default OwnChannelCommentaryPage
